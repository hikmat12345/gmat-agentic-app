...
import { saveGmatQuizSession } from "@/lib/db/queries/gmat-quiz";
...
export async function POST(req: Request) {
  ...
  const session = await saveGmatQuizSession({
    userId: user.id,
    subtopicId,
    score,
    totalQuestions,
    ...
    if (subtopicData) {
      ...
      if (skill) {
        ...
        for (const answer of answers) {
          ...
          const mutations = updateSkillAfterAnswer(currentSkill, answer.isCorrect, diffLevel);
